...
export class FirebaseStorage implements IStorage {
  ...
  async createAttendanceLog(log: InsertAttendanceLog & { selectedStore?: string, photoPath?: string }): Promise<AttendanceLog> {
    ...
    const getBratislavaTime = async () => {
      // Primary source: Internationalization API with explicit timezone
      // This is the most robust way to get local time on serverless platforms like Vercel
      const d = new Date();
      try {
        ...
        console.error("Intl failure, using WorldTimeAPI as backup:", e);
      }
      ...
      try {
        ...
        console.error("WorldTimeAPI backup failure, using raw server time:", e);
      }
